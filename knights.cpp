...
1. **Forced Move Pruning (Leaf Detection):**
   During DFS, if a move leads to a position where a neighbor has no further moves (a leaf), that move must be taken immediately. If multiple such forced moves exist, the path is invalid. This drastically reduces the search space by eliminating paths that would inevitably fail.
2. **Small-Stride Orientation (Canonicalization):**
   By ensuring the board is oriented such that the smaller dimension is treated as the width (N), we minimize the bit-distance between knight moves in memory. This improves cache locality and the effectiveness of heuristic move ordering.
3. **Bit-Parallel Connectivity Checks:**
   Using bitwise operations to perform connectivity checks allows for rapid determination of whether the remaining unvisited nodes form a connected subgraph. This is crucial for pruning invalid states early in the search.
4. **D2/D4 Symmetry Reduction:**
   By identifying canonical starting positions under the board's symmetry group, we reduce redundant computations. Each unique starting position is weighted by the size of its orbit, allowing us to count all equivalent paths without explicitly exploring them.
5. **Parallelization:**
   The outer loop over canonical starting positions is parallelized, allowing multiple DFS searches to occur simultaneously. This takes advantage of multi-core processors to significantly speed up the overall computation.
*/
...
struct Solver {
    ...
    u64 dfs(int current, u64 visited) const {
        ...
        while (moves_mask) {
            ...
            if (deg == 0) {
                if (forced_move != -1) return 0; // Fork -> Fail
                forced_move = v;
            }
